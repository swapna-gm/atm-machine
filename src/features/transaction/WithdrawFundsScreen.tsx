...
export default function WithdrawScreen() {
  ...
  const rightButtons = [
    ...
    { enabled: false },
    { enabled: false },
    { enabled: true, text: "Withdraw", ariaLabel: 'Withdraw', onClick: () => handleWithdraw() },
  ];
  ...
  async function handleWithdraw() {
    ...
    setLoading(true);
    try {
      const newBal = await atmService.withdraw(numAmount);
      dispatch({ type: 'SET_BALANCE', payload: newBal });
      setLoading(false);
      navigate('/success', { replace: true });
    } catch (e: any) {
      setLoading(false);
      setLocalError(e || 'Withdraw failed');
      setAmount('');
    }
  }
  ...
  return (
    <ATMFrame leftButtons={leftButtons} rightButtons={rightButtons}>
      ...
      >
        <div className="font-retro text-lg mb-3 text-center">Withdraw Funds</div>
        <form
          onSubmit={e => {
            e.preventDefault();
            if (!loading) handleWithdraw();
          }}
